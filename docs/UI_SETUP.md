...
### OpenAI 호환 (KISTI, LiteLLM proxy, NVIDIA API 등)
...
모델에 따라 `top_p`, `top_k` 추가 파라미터도 지원한다 (NVIDIA API 등):
...
현재 실제 구성:
```
host/beginner/config.json   → provider: "openai"     (kistillm, KISTI LiteLLM proxy, temp=0.5)
host/developer/config.json  → provider: "openai"     (stepfun-ai/step-3.5-flash, NVIDIA API, temp=1.0)
```
