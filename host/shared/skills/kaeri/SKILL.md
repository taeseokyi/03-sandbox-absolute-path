...
allowed-tools: Read, Write, Bash(python -m shared.skills.kaeri.main *)
---
...
2. 아래 명령으로 변환 실행:
   ```
   python -m shared.skills.kaeri.main --source <경로_또는_URL> --output kaeri_dataon.json
   ```
...
입력: `python -m shared.skills.kaeri.main --source https://oard.kaeri.re.kr/api/datasets --output out.json`
출력: DataON 등록 JSON 파일 (`out.json`)

입력: `python -m shared.skills.kaeri.main --source nuclear_data.xlsx`
