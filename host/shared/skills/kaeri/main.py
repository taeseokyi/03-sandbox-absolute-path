"""
KAERI (한국원자력연구원) DataON 등록 변환 스킬
...
def main() -> None:
    parser = argparse.ArgumentParser(description="KAERI 연구데이터 → DataON 등록 JSON 변환")
    ...
    if not records:
        print("[kaeri] 경고: 소스에서 레코드를 찾을 수 없습니다.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output:
        ...
        print(f"[kaeri] 저장 완료: {args.output}  (레코드 {len(records)}건 처리)")
    else:
        print(result)
