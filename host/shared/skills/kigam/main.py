"""
KIGAM (한국지질자원연구원) DataON 등록 변환 스킬
...
from skills.kigam.utils import load_source, build_dataon_form


def main() -> None:
    parser = argparse.ArgumentParser(description="KIGAM 연구데이터 → DataON 등록 JSON 변환")
    ...
    if not records:
        print("[kigam] 경고: 소스에서 레코드를 찾을 수 없습니다.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.output:
        ...
        print(f"[kigam] 저장 완료: {args.output}  (레코드 {len(records)}건 처리)")
    else:
        print(result)
