...
allowed-tools: kisti-mcp_search_scienceon_papers, kisti-mcp_search_scienceon_paper_details, kisti-mcp_search_scienceon_patents, kisti-mcp_search_scienceon_patent_details, kisti-mcp_search_scienceon_patent_citations, kisti-mcp_search_scienceon_reports, kisti-mcp_search_scienceon_report_details, kisti-mcp_search_ntis_rnd_projects, kisti-mcp_search_ntis_science_tech_classifications, kisti-mcp_search_ntis_related_content_recommendations, kisti-mcp_search_dataon_research_data, kisti-mcp_search_dataon_research_data_details
---
...
search_scienceon_papers(query="북극해 해저지질", max_results=10)
# 반환: { query, total_count, count, papers: [{cn, title, author, ...}] }

# 검색 결과의 cn으로 상세 조회
search_scienceon_paper_details(cn="<논문 CN번호>")
...
```python
# 키워드로 목록 검색
search_scienceon_patents(query="수소연료전지", max_results=10)
# 반환: { query, total_count, count, patents: [{cn, title, applicant, ...}] }

# 검색 결과의 cn으로 상세/인용 조회
search_scienceon_patent_details(cn="<특허 CN번호>")
search_scienceon_patent_citations(cn="<특허 CN번호>")
...
```python
search_scienceon_reports(query="탄소중립 해양", max_results=10)
# 반환: { query, total_count, count, reports: [{cn, title, organization, ...}] }

search_scienceon_report_details(cn="<보고서 CN번호>")
...
search_ntis_rnd_projects(query="극지연구소 해저지질", max_results=10)
search_ntis_rnd_projects(query="1525008151")          # 10자리 과제번호 직접 조회
search_ntis_rnd_projects(query="홍종국 2024 북극해")  # 이름+연도+주제 조합
# 반환: { query, total_count, count, projects: [{ProjectNumber, ProjectTitle, ...}] }

# 연관 콘텐츠 추천 — pjt_id는 projects[].ProjectNumber (숫자여도 문자열로 전달)
search_ntis_related_content_recommendations(pjt_id="1425118980", max_results=15)
...
search_dataon_research_data(query="기후변화 북극", max_results=10)
search_dataon_research_data(query="이승우", max_results=20, from_pos=0)   # 페이징
search_dataon_research_data(query="해저지질", sort_con="date", sort_arr="desc")
# max_results 최대 100, from_pos로 페이징, sort_con: "date"|"title"|"" (관련도순)
# 반환: API 원본 필드 전체 (svc_id 포함)

# 검색 결과의 svc_id로 상세 조회
search_dataon_research_data_details(svc_id="<svcId>")
...
### 패턴 A — 분류코드 자동 부여
```
1. search_ntis_science_tech_classifications(query=<연구과제 초록>) 또는
   search_ntis_science_tech_classifications(korean_keywords=<키워드>, research_content=<연구내용>)
2. 반환된 분류코드 → DataON JSON `기본.과학기술표준분류` 필드 삽입
```

### 패턴 B — 관련과제 연결
```
1. search_ntis_rnd_projects(query="<기관명> <연도> <연구주제>")
...
### 패턴 C — DataON 중복 검사
```
1. search_dataon_research_data(query=<데이터셋 제목>)
...
```
1. search_scienceon_papers(query="<기관명> <연구주제>")
2. search_scienceon_patents(query="<기관명> <기술명>")
